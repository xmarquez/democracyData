...
VDem and [LIED](../../reference/LIED.html) code the largest total number of country-years, while specialist indexes like [BLM](../../reference/blm.html) (which covers only 5 countries in Latin America) code the smallest number. Given the increase in the number of states in the state system, it's no surprise that coverage increases greatly after WWII, with many datasets covering a large number of countries. 
