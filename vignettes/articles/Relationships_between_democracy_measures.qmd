...
#| fig-cap: "Distribution of pairwise correlation coefficients among dichotomous measures of democracy."
...
#| fig-cap: "Average pairwise correlation among all democracy measures, by year, with 95% confidence intervals."
...
correlations_summary <- correlations |>
  ...
  summarise(
    ...
    r_sd = sd(r),
    r_se = r_sd / sqrt(n),
    r_lower = dplyr::if_else(
      n > 1,
      r - stats::qt(0.975, df = n - 1) * r_se,
      NA_real_
    ),
    r_upper = dplyr::if_else(
      n > 1,
      r + stats::qt(0.975, df = n - 1) * r_se,
      NA_real_
    ),
...
correlations_summary |>
  ...
  geom_rect(data = wars, aes(xmin = xmin, 
                      xmax = xmax,
                      ymin = -Inf, ymax = Inf), fill = "red", alpha = 0.3) +
  geom_ribbon(aes(x = year, ymin = r_lower, ymax = r_upper), alpha = 0.2) +
