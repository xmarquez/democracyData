...
test_that("Freedom House electoral democracies data downloads correctly", {
  ...
  later_urls <- lapply(urls[-1], \(x) {
    read_data(x, verbose = FALSE, skip = 1)
  }) |>
    lapply(function(x) {
  ...
  for (i in 1:length(later_urls)) {
    later_urls[[i]] <- later_urls[[i]] |>
      mutate(
        ...
        country = case_when(
          .data$Country == "Yemen, S." ~ "South Yemen",
  ...
  from_spreadsheets <- bind_rows(later_urls) |>
    mutate(year = case_when(year < 2021 ~ year - 1, TRUE ~ year)) |>
    country_year_coder(
      country,
      year,
  ...
  # Upstream FH discrepancy: 4 country-years for 2023 (El Salvador, Georgia,
  # Kenya, Madagascar) are marked non-electoral in the per-report xlsx lists but
  # appear as electoral in the aggregate classification. Tracked against FH, not
  # our code.
  expect_equal(
    nrow(
      from_spreadsheets |>
        left_join(fh_electoral_downloaded) |>
        filter(electoral_fs != electoral)
    ),
    4
  )
  expect_equal(
    fh_electoral_downloaded |>
