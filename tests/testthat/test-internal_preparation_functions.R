...
test_that("vdem_simple contains all countries and is the right version", {
  expect_true(rlang::is_installed("vdemdata", version = "16.0"))
  vdem_simple_prepared <- prepare_vdem_simple(version = "16.0")
  ...
  expect_equal(
    vdem_simple_prepared |>
      filter(duplicated(paste(
        vdem_simple_prepared$vdem_country_name,
        vdem_simple_prepared$year
      ))) |>
      nrow(),
    0
  )
  ...
  expect_true(all(
    vdem_simple_prepared$extended_country_name != "Piedmont-Sardinia"
  ))
