...
* **V-Dem v16.** Bumped `vdem_simple` and the `vdemdata` Suggests version to 16.0 (V-Dem v16, released 2026-03-17).

* **BTI 2026.** Updated `bti` to the Bertelsmann Transformation Index 2026 edition (data through 2024).

* **Bjørnskov-Rode v6.2.** Updated `pacl_update` to the September 2025 release (v6.2). This is a substantial jump from v3.2; column structure may differ, verify downstream users of this dataset.
...
* **Freedom House.** Freedom House moved their machine-readable FIW data to email-request distribution in 2026. `fh`, `fh_full`, and `fh_electoral` therefore remain frozen at the 2024 report / 2025 release; we will re-enable bumps once public URLs return or once we have a reliable fallback.

* **Anckar** is unchanged. Harvard Dataverse shows a v3.0 release, but the underlying data file (`anckarfredrikssonepsdatafinal2.0`) is identical to v2.0; only a new codebook PDF was added.

* **LIED 6.9.** Updated `LIED` to version 6.9 using the April 1, 2026 Harvard Dataverse file. The upstream schema is unchanged, and the package now includes the additional 2025 country-year observations.
...
* **VaPoReg (Kailitz).** This is now an explicit breaking change. The shipped `vaporeg` object contains the March 2026 VaPoReg release in its current upstream schema (data through 2025), and `redownload_vaporeg()` returns that upstream country-year table directly rather than translating it back into legacy `democracyData` variable names. The old packaged interface is preserved separately as `vaporeg_2024`. Internally, `generate_democracy_scores_dataset()` now derives its VaPoReg democracy measures from the new release using VaPoReg's own simplified classifications (`vaporeg_regtype_bindem` and `vaporeg_regtype_triple`) plus a package-specific strict democracy indicator.
...
* A small number of successor-state country-year rows that previously fell into the "between-periods" gap may now resolve differently as a side effect of the bug fix. Behavior on the shipped golden regression fixture is unchanged.
