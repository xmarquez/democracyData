...
#'   states (<https://web.archive.org/web/20130627160240/http://privatewww.essex.ac.uk/~ksg/statelist.html>).
#'   This is very
...
#'   states (\url{https://correlatesofwar.org/data-sets/cow-country-codes/}). This
...
#'   dataset (\url{https://www.systemicpeace.org/polity/polity4.htm}). This is very
...
#'   these systems.
#'
#'   \item "eurostat": Eurostat 2-letter country code. Taken from [countrycode::countrycode].
#'
#'   \item "eu28": Whether the state is a member of the European Union (as of
#'   December 2015), without special territories. Taken from [countrycode::countrycode].
#'
#'   \item "fao": Food and Agriculture Organization of the United Nations numerical
#'   country code. Taken from [countrycode::countrycode].
#'
...
#'   Taken from [countrycode::countrycode].
#'
...
#'   [countrycode::countrycode].
#'
#'   \item "iso2c": ISO-2 character. Taken from [countrycode::countrycode].
#'
#'   \item "iso3c": ISO-3 character. Taken from [countrycode::countrycode].
#'
#'   \item "iso2n": ISO-2 numeric. Taken from [countrycode::countrycode].
#'
#'   \item "iso3n": ISO-3 numeric. Taken from [countrycode::countrycode].
#'
#'   \item "un": United Nations numerical code. Taken from [countrycode::countrycode].
#'
#'   \item "wb": World Bank (very similar but not identical to iso3c). Taken from
#'   [countrycode::countrycode].
#'
#'   \item "wb_api2c": World Bank API 2 character code. Taken from [countrycode::countrycode].
#'
#'   \item "wb_api3c": World Bank API 3 character code. Taken from [countrycode::countrycode].
#'
#'   \item "wvs": World Values Survey numeric code. Taken from [countrycode::countrycode].
#'   }
#'
...
#'   \url{https://correlatesofwar.org/data-sets/cow-country-codes/} for details. Is
...
#'   datasets.
#'
#'   \item "GWc": Three letter code for the country in the Gleditsch-Ward system
#'   of states
#'   (<https://web.archive.org/web/20130627160240/http://privatewww.essex.ac.uk/~ksg/statelist.html>)
#'   (Gleditsch and War 1999). This is very
#'   similar, but not identical, to the Correlates of War and Polity codes. See
...
#'
#'   \item "cown": Numeric code for the country in the Correlates of War system of
#'   states (\url{https://correlatesofwar.org/data-sets/cow-country-codes/}), 2016
...
#'   Indicators. Taken from [countrycode::countrycode].
#'
...
#'   His tentative list of microstates is available via the Wayback Machine at
#'   <https://web.archive.org/web/20130627160240/http://privatewww.essex.ac.uk/~ksg/statelist.html>.
#'
...
#'   region. Taken from [countrycode::countrycode].
#'
#'   \item "eurocontrol_statfor": European Organisation for the Safety of Air
#'   Navigation region. Taken from [countrycode::countrycode].
#'
#'   \item "eurostat": Eurostat 2-letter country code. Taken from [countrycode::countrycode].
#'
#'   \item "eu28": Whether the state is a member of the European Union (as of
...
#'   \item "wvs": World Values Survey numeric code. Taken from [countrycode::countrycode].
#'
#'   \item "country_name_ar": country name (Arabic). Taken from [countrycode::countrycode].
#'
#'   \item "country_name_de": country name (German). Taken from [countrycode::countrycode].
#'
#'   \item "country_name_es": country name (Spanish). Taken from [countrycode::countrycode].
#'
#'   \item "country_name_fr": country name (French). Taken from [countrycode::countrycode].
#'
#'   \item "country_name_ru": country name (Russian). Taken from [countrycode::countrycode].
#'
#'   \item "country_name_zh": country name (Chinese). Taken from [countrycode::countrycode].
#'   }
#'
...
country_year_coder <- function(
  ...
  if (missing(date_col)) {
    stop(
      "Missing date_col argument. If you don't care about date matching",
  ...
  if (match_type == "code only") {
    ...
    result_df <- result_df |>
      mutate(
        ...
        matches_code_year = ifelse(
          is.na(matches_code_year),
          FALSE,
          matches_code_year
        ),
  ...
  result_df <- result_df |>
    mutate(
      in_GW_system = matches_code_year_GW &
        GW_membership |
        matches_country_year_GW & GW_membership,
      in_polity_system = matches_code_year_polity &
        polity_membership |
        matches_country_year_polity & polity_membership,
      in_cow_system = matches_code_year_cow &
        cow_membership |
  ...
  if (!missing(code_col) & !missing(country_col)) {
    result_df <- result_df |>
      select(
        ...
        !!code_col,
        !!date_col,
        all_of(include_in_output),
        starts_with("matches_")
      )
  } else if (!missing(code_col) & missing(country_col)) {
    result_df <- result_df |>
      select(
  ...
  if (verbose) {
    if (!missing(country_col) & missing(code_col)) {
      print_diagnostic(tbl, country_col = !!country_col, date_col = !!date_col)
      ...
      print_diagnostic(tbl, code_col = !!code_col, date_col = !!date_col)
    }
  }
...
code_year_match <- function(
  ...
  code_matches <- code_matches |>
    mutate(
      matches_code_year = year_match_fun(!!date_col, startdate, enddate),
      matches_code_year_GW = year_match_fun(
        !!date_col,
        GW_startdate,
        GW_enddate
      ),
      matches_code_year_polity = year_match_fun(
        !!date_col,
        polity_startdate,
        polity_enddate
      ),
      matches_code_year_cow = year_match_fun(
        !!date_col,
        cow_startdate,
        cow_enddate
      ),
      matches_code_year_unofficial = year_match_fun(
        !!date_col,
        unofficial_startdate,
        unofficial_enddate
      ),
      matches_before_code_year = is_before(!!date_col, startdate) &
        !matches_code_year,
      matches_before_code_year_GW = is_before(!!date_col, GW_startdate) &
        !matches_code_year_GW,
      matches_before_code_year_cow = is_before(!!date_col, cow_startdate) &
        !matches_code_year_cow,
      matches_before_code_year_polity = is_before(
        !!date_col,
        polity_startdate
      ) &
        !matches_code_year_polity,
      matches_between_code_year = is_between(!!date_col, startdate, enddate) &
        !matches_code_year,
      matches_between_code_year_GW = is_between(
        !!date_col,
        GW_startdate,
        GW_enddate
      ) &
        !matches_code_year_GW,
      matches_between_code_year_cow = is_between(
        !!date_col,
        cow_startdate,
        cow_enddate
      ) &
        !matches_code_year_cow,
      matches_between_code_year_polity = is_between(
        !!date_col,
        polity_startdate,
        polity_enddate
      ) &
      ...
      matches_after_code_year_polity = distance_after(
        !!date_col,
        polity_enddate
      ),
...
country_year_match <- function(
  ...
  # Two-sided normalization: strip diacritics/punctuation from the user's
  # country column into `.country_norm`, and rewrite the dict's regex column
  # so patterns like `u\.?s\.?a\.?` and `türkiye` still match. The user's
  # original country column is preserved untouched for the final left_join
  # back to the input tibble in `country_year_coder()`.
  data <- data |>
    mutate(.country_norm = normalize_country(!!country_col))

  dict <- dict |>
    mutate(regex = normalize_dict_regex(regex))

  join_by <- c(".country_norm" = "regex")

  suppressWarnings(
    country_matches <- data |>
      fuzzyjoin::regex_full_join(dict, by = join_by, ignore_case = TRUE) |>
  ...
  if (!missing(code_col)) {
    ...
    country_matches <- country_matches |>
      mutate(
        matches_code = (UQ(code_col) == UQ(code_type)),
        matches_code = ifelse(is.na(matches_code), FALSE, matches_code)
      )
  } else {
    country_matches <- country_matches |>
  ...
  country_matches <- country_matches |>
    mutate(
      matches_country_year = year_match_fun(!!date_col, startdate, enddate),
      matches_country_year_GW = year_match_fun(
        !!date_col,
        GW_startdate,
        GW_enddate
      ),
      matches_country_year_polity = year_match_fun(
        !!date_col,
        polity_startdate,
        polity_enddate
      ),
      matches_country_year_cow = year_match_fun(
        !!date_col,
        cow_startdate,
        cow_enddate
      ),
      matches_country_year_unofficial = year_match_fun(
        !!date_col,
        unofficial_startdate,
        unofficial_enddate
      ),
      ...
      matches_code_year_unofficial = matches_code &
      ...
      matches_before_country_year_polity = is_before(
        !!date_col,
        polity_startdate
      ),
      matches_between_country_year = is_between(
        !!date_col,
        startdate,
        enddate
      ) &
        !matches_country_year,
      matches_between_country_year_GW = is_between(
        !!date_col,
        GW_startdate,
        GW_enddate
      ) &
        !matches_country_year_GW,
      matches_between_country_year_cow = is_between(
        !!date_col,
        cow_startdate,
        cow_enddate
      ) &
        !matches_country_year_cow,
      matches_between_country_year_polity = is_between(
        !!date_col,
        polity_startdate,
        polity_enddate
      ) &
      ...
      matches_after_country_year_polity = distance_after(
        !!date_col,
        polity_enddate
      ),
...
year_match_fun <- function(date_col, startdate, enddate) {
  ...
  result <- ymd(paste0(date_col, "-12-31")) %within%
    interval(startdate, case_when(is.na(enddate) ~ today, TRUE ~ enddate))

  # Replace any NA values in result with FALSE
  case_when(is.na(result) ~ FALSE, TRUE ~ result)
}
...
is_before <- function(date_col, startdate) {
  ...
  result <- ymd(paste0(date_col, "-12-31")) <
    case_when(is.na(startdate) ~ today, TRUE ~ startdate)

  # Replace any NA values in result with FALSE
  case_when(is.na(result) ~ FALSE, TRUE ~ result)
}

# Internal: returns TRUE when `date_year` (integer year) falls in the gap
# between the earliest and latest period of an entity.
#
# Implementation preserves the semantics used in country_year_coder through
# v0.6.0: `target > min(startdate) & target < max(startdate) & target > min(enddate)`.
# For 2-period entities (USSR/Russia, East/West Germany, Czechoslovakia splits)
# this is equivalent to a proper gap check. For 3+-period entities it can flag
# dates inside an intermediate period as "between"; this edge case is accepted
# as existing behavior. A stricter per-gap check should be introduced only with
# updated regression fixtures so the shift is explicit.
date_year_in_gap <- function(date_year, startdate, enddate) {
  target <- ymd(paste0(date_year, "-12-31"))
  # NA startdate/enddate yields NA via min()/max(); callers coerce to FALSE
  # (is_between) or substitute today's date for NAs upstream (test_condition),
  # matching the behavior that shipped through v0.6.0.
  target > min(startdate) &
    target < max(startdate) &
    target > min(enddate)
}
...
is_between <- function(date_col, startdate, enddate) {
  result <- tibble(
    ...
    group_by(date_year) |>
    mutate(between = date_year_in_gap(date_year, startdate, enddate)) |>
    pull(between)
...
distance_after <- function(date_col, enddate) {
  ...
  result <- ymd(paste0(date_col, "-12-31")) -
    case_when(is.na(enddate) ~ today, TRUE ~ enddate)
...
#'
#' Applies the transformations [country_year_coder()] uses on user-supplied
#' country strings before the fuzzy regex join: transliterate to ASCII (to
#' drop diacritics), lowercase, replace runs of punctuation with a single
#' space, collapse whitespace, and trim.
#'
...
#' @return A normalized character vector, same length as `x`.
#' @examples
...
# Internal: rewrite a dictionary regex so it still matches country names that
# have been put through `normalize_country()`.
#
# Transformations:
#   1. Transliterate literal diacritics to ASCII (`türkiye` -> `turkiye`,
#      `(a|ã)` -> `(a|a)`, `(e|é)` -> `(e|e)`). Regex metacharacters are
#      ASCII so Latin-ASCII leaves them untouched.
#   2. Rewrite the acronym dot-tolerance pattern `\.?` to `\s*` so
#      `u\.?s\.?a\.?` matches both "usa" and "u s a" (punctuation in the
#      input is mapped to a single space by `normalize_country()`).
#   3. Rewrite literal punctuation that `normalize_country()` would collapse
#      to whitespace: commas followed by whitespace (preserving `{N,M}`
#      quantifiers), apostrophes, and hyphens. Hyphens are blanket-rewritten;
#      the current dictionary uses only `[\\.]` as a character class, so no
#      hyphen appears inside `[...]`.
#
# Everything else (word boundaries, lookarounds, alternation, char classes)
# is preserved as-is.
normalize_dict_regex <- function(regex) {
  out <- stringi::stri_trans_general(regex, "Any-Latin; Latin-ASCII")
  out <- gsub("\\\\\\.\\?", "\\\\s*", out)
...
prepare_dict <- function(to_system, match_final_year) {
  ...
  if (match_final_year) {
    dict <- dict |>
      mutate(across(
        c("GW_enddate", "cow_enddate", "polity_enddate", "unofficial_enddate"),
        \(x) ceiling_date(x, unit = "year") - days(1)
      ))
  }
  ...
  dict <- dict |>
    mutate(
      enddate = if_else(is.na(enddate) & !is.na(startdate), today, enddate)
    ) |>
    group_by(pick(everything())) |>
    mutate(
      interval = list(c(startdate, enddate)),
...
print_diagnostic <- function(
  ...
  matched_country_col <- parse_expr(matched_cols[grepl(
    "country",
    matched_cols
  )][1])
  ...
  if (missing(country_col)) {
    ...
    multiple_matches <- tbl |>
      ...
      summarise(
        min_date = suppressWarnings(min(!!date_col)),
  ...
  if (!missing(country_col) & !missing(code_col)) {
    country_name_changes <- tbl |>
      filter(
        stringr::str_to_lower(!!country_col) !=
  ...
  if (nrow(multiple_matches_2) > 0) {
    if (print_as_table) {
      ...
      print(
        knitr::kable(multiple_matches_2),
  ...
  if (nrow(country_name_changes) > 0) {
    if (print_as_table) {
      ...
      print(
        knitr::kable(country_name_changes),
...
test_condition <- function(
  ...
  GW_startdate <- case_when(
    is.na(GW_startdate) ~ as.Date(lubridate::now()),
    TRUE ~ GW_startdate
  )

  GW_enddate <- case_when(
    is.na(GW_enddate) ~ as.Date(lubridate::now()),
    TRUE ~ GW_enddate
  )

  cow_startdate <- case_when(
    is.na(cow_startdate) ~ as.Date(lubridate::now()),
    TRUE ~ cow_startdate
  )

  cow_enddate <- case_when(
    is.na(cow_enddate) ~ as.Date(lubridate::now()),
    TRUE ~ cow_enddate
  )

  polity_startdate <- case_when(
    is.na(polity_startdate) ~ as.Date(lubridate::now()),
    TRUE ~ polity_startdate
  )

  polity_enddate <- case_when(
    is.na(polity_enddate) ~ as.Date(lubridate::now()),
    TRUE ~ polity_enddate
  )

  before_GW <- unique(ymd(paste0(date_col, "-12-31"))) <
    min(GW_startdate, na.rm = TRUE)
  before_cow <- unique(ymd(paste0(date_col, "-12-31"))) <
    min(cow_startdate, na.rm = TRUE)
  before_polity <- unique(ymd(paste0(date_col, "-12-31"))) <
    min(polity_startdate, na.rm = TRUE)

  before_return <- TRUE

  if (before_GW) {
    # Country year falls before the entry of the country into the system
    before_return <- (GW_startdate == min(GW_startdate, na.rm = TRUE)) &
      before_return
  }

  if (before_cow) {
    # Country year falls before the entry of the country into the system
    before_return <- (cow_startdate == min(cow_startdate, na.rm = TRUE)) &
      before_return
  }

  if (before_polity) {
    # Country year falls before the entry of the country into the system
    before_return <- (polity_startdate == min(polity_startdate, na.rm = TRUE)) &
      before_return
  }
  ...
  after_GW <- unique(ymd(paste0(date_col, "-12-31"))) >
    max(GW_enddate, na.rm = TRUE)
  after_cow <- unique(ymd(paste0(date_col, "-12-31"))) >
    max(cow_enddate, na.rm = TRUE)
  after_polity <- unique(ymd(paste0(date_col, "-12-31"))) >
    max(polity_enddate, na.rm = TRUE)

  if (after_GW | after_cow | after_polity) {
    # Country year falls after the exit of the country into the system
    return(
      (GW_enddate == max(GW_enddate, na.rm = TRUE)) &
  ...
  startdate <- case_when(
    is.na(startdate) ~ as.Date(lubridate::now()),
    TRUE ~ startdate
  )

  enddate <- case_when(
    is.na(enddate) ~ as.Date(lubridate::now()),
    TRUE ~ enddate
  )

  if (any(date_year_in_gap(date_col, startdate, enddate))) {
  ...
  between_GW <- any(date_year_in_gap(date_col, GW_startdate, GW_enddate))
  between_cow <- any(date_year_in_gap(date_col, cow_startdate, cow_enddate))
  between_polity <- any(date_year_in_gap(
    date_col,
    polity_startdate,
    polity_enddate
  ))
