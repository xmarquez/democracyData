#' Extended UDS
#'
#' @format A [tibble::tibble()] with
#'   `r local({ load("data/extended_uds.rda"); nrow(extended_uds) })` rows and
#'   20 columns.
#'
...
#' the latest democracy datasets included in `democracyData` through
#' `r local({ load("data/extended_uds.rda"); max(extended_uds$year) })`, and
#' calculating scores for countries not in the official UD release. Cite both
#' Pemstein, Meserve, and Melton 2010 as well as Marquez 2016.
#'
#' @section Overview:
#'
...
#'   `r local({ load("data/extended_uds.rda"); nrow(extended_uds) })`
#'   country-years
#'   (`r local({ load("data/extended_uds.rda"); length(unique(extended_uds$extended_country_name)) })`
#'   distinct countries and non-sovereign territories). It is generated by the
#'   function [generate_extended_uds].
#'
...
#'   the following datasets: [anckar], [anrr], [arat], [blm], [bmr]
#'   (`democracy_femalesuffrage` indicator only), [bnr_extended], [bti],
#'   [bollen_pmm], [svmdi] (continuous indicator), [doorenspleet], [eiu], [fh]
#'   (`fh_total_reversed` indicator), [fh_electoral], [gwf_all_extended],
#'   [hadenius_pmm], [LIED], [magaloni_extended], [mainwaring], [munck_pmm],
#'   [pacl], [pacl_update], [peps] (PEPS1v indicator), [pitf] (non-binary
#'   indicator), [polity] (polity 2 indicator), [polyarchy] (both contestation
#'   and participation dimensions), [polyarchy_dimensions] (both contestation
#'   and participation dimensions), [prc], [REIGN], [svolik],
#'   [ulfelder_extended], [utip] (trichotomous indicator, with one-party
#'   democracy as the middle category), the `v2x_polyarchy` indicator from
#'   [vdem_simple], [vanhanen] (the `vanhanen_democratization` indicator only),
#'   [vaporeg] (the `vaporeg_trichotomous` indicator), [wgi], and
#'   [wahman_teorell_hadenius] (the `wth_democrobust` variable). Please refer
#'   to the documentation of [generate_democracy_scores_dataset] for more
...
#'   `r local({ load("data/extended_uds.rda"); min(extended_uds$num_measures) })`
#'   to
#'   `r local({ load("data/extended_uds.rda"); max(extended_uds$num_measures) })`;
#'   one can filter the dataset to include only scores where more than, say, 5
