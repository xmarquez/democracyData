prepare_pmm_replication_data <- function(
  ...
  if (verbose) {
    message(sprintf("Original dataset has %d rows.", nrow(democracy)))
  ...
  vars <- names(pmm)[
    which(names(pmm) == "arat"):which(names(pmm) == "vanhanen")
  ]
  ...
  if (verbose) {
    ...
    if (nrow(pmm) != nrow(democracy)) {
      message(
        "Note: the number of rows in the processed PMM data is different from the number of rows in the original data."
      )
    }
  }
...
extract_pmm_var <- function(
  pmm_replication_data,
  pmm_var,
  verbose = FALSE,
  include_in_output
) {
...
prepare_mainwaring <- function(
  path = "../../Data/Mainwaring Linan.txt",
  verbose = TRUE,
  ...
) {
  if (verbose) {
  ...
  if (verbose) {
    ...
    if (nrow(mainwaring) != nrow(data)) {
      message(
        "Note: the number of rows in the processed data is",
  ...
  standardize_columns(mainwaring, Country, verbose = verbose)
}

prepare_eiu <- function(
  path = "data-raw/eiu_wikipedia_snapshot.csv",
  verbose = TRUE,
  ...
) {
  if (verbose) {
  ...
  # The EIU Democracy Index 2025 report (Dec 2025) no longer publishes a full
  # country-by-country appendix, so the per-country panel is scraped from
  # Wikipedia's "The Economist Democracy Index" article, which aggregates all
  # years 2006-latest from the EIU's published reports. See
  # data-raw/scrape_eiu_wikipedia.R for the refresh script.
  data <- readr::read_csv(snapshot_path, show_col_types = FALSE)

  # Harmonise country labels to the dictionary used by country_year_coder
  # (matches the conventions used elsewhere in this package).
  data <- data |>
    mutate(
      country = case_when(
  ...
  if (verbose) {
    message(sprintf(
      "Original dataset has %d rows and is in country-year format",
      nrow(data)
    ))
  ...
  eiu <- data |>
    ...
    # Preserve the legacy convention of "Saudi" for the short country label
...
prepare_doorenspleet <- function(
  path = "../../Data/Doorenspleet data.csv",
  verbose = TRUE,
  ...
) {
  if (verbose) {
  ...
  if (verbose) {
    ...
    message(
      "Processing the Doorenspleet data - turning to country-year format, ",
  ...
  doorenspleet <- data |>
    ...
    group_by(country) |>
    mutate(
      end_year_2 = lead(start_year) - 1,
      end_year_2 = ifelse(is.na(end_year), end_year_2, as.numeric(end_year)),
      end_year_2 = ifelse(is.na(end_year_2), 1994, end_year_2),
      end_year = end_year_2
    ) |>
    select(-end_year_2) |>
...
prepare_prc <- function(
  path = "../../Data/Gasiorowski.csv",
  verbose = TRUE,
  ...
) {
  data <- read_data(
  ...
  prc_gasiorowski <- data |>
    ...
    mutate(prc_at_end_year = last(prc), prc_at_beginning_year = first(prc)) |>
    ungroup() |>
  ...
  if (verbose) {
    ...
    if (nrow(data) != nrow(prc_gasiorowski)) {
      message(
        "Note: the number of rows in the processed Gasiorowski data is ",
        "different from the number of rows in the original data."
      )
    }
  }
...
create_pitf_scores <- function(
        ...
        ifelse(
          ...
          ifelse(
            ...
            ifelse(
              ...
              "3-Partial democracy",
              ifelse(
                exrec >= 6 & parcomp %in% c(3),
                "2-Partial democracy with factionalism",
                "4-Full democracy"
              )
            )
          )
  ...
  if (verbose) {
    message(sprintf(
      "Resulting data has %d rows, fewer than Polity dataset, because of exclusion of interruption codes",
      nrow(pitf)
    ))
  }
...
prepare_svolik_regime <- function(
  ...
  data <- read_data(path, verbose = verbose, name = 'Svolik regime data')
...
prepare_svolik_institutions <- function(
  ...
  data <- read_data(path, verbose = verbose, ...)
...
prepare_vanhanen <- function(
  ...
  if (verbose) {
    message(sprintf(
      "Converted to country-year format, the dataset has %d rows",
      nrow(data)
    ))
  }
...
prepare_anrr <- function(
  ...
  anrr <- anrr |>
    ...
    dplyr::mutate(
      country_name_matched = case_when(
        country_name %in% "Serbia & Montenegro" & year > 2006 ~ "Serbia",
        str_detect(country_name, "ncipe") ~ "Sao Tome and Principe",
        str_detect(country_name, "d'Ivoire") ~ "Cote d'Ivoire",
        TRUE ~ country_name
      )
    ) |>
    country_year_coder(country_name_matched, year, verbose = verbose, ...) |>
    dplyr::select(-country_name_matched)

  standardize_columns(anrr, country_name, wbcode, verbose = verbose)
}

prepare_vdem_simple <- function(verbose = TRUE, version = "16.0", ...) {
  ...
  vdem_simple <- vdem_simple |>
    dplyr::rename(vdem_country_name = country_name, vdem_cowcode = COWcode) |>
    mutate(
...
prepare_arat <- function(path, verbose = TRUE, include_in_output) {
  arat <- readr::read_csv(path) |>
    rename(arat_dem = Score, year = Year, country = Country) |>
    mutate(
      country = case_when(
        country == "Germany DR" ~ "East Germany",
  ...
  arat <- arat |>
    ...
    filter(
      !(extended_country_name == "Brazil" & year == 1981),
