...
kailitz <- read_rds("kailitz.yearly.rds") %>%
...
usethis::use_data(REIGN, overwrite = TRUE)
```
...
# vanhanen <- prepare_vanhanen(path = here::here("data-raw/daF1289e.xls"),
#                              verbose = verbose,
#                              include_in_output = include_in_output)
...
vanhanen <- prepare_vanhanen(here::here("data-raw/FSD1289/FSD1289/Study/data/daF1289e.csv"),
                                  include_in_output = include_in_output)
...
vdem_simple <- vdemdata::vdem %>%
  select(country_name:v2x_mpi_sd) %>%
  as_tibble() %>%
  country_year_coder(country_name, year, COWcode,
                     include_in_output = include_in_output) 

vdem_exclude <- vdemdata::vdem %>%
  select(country_name:v2x_mpi_sd) %>%
  as_tibble() %>%
  country_year_coder(country_name, year,
                     include_in_output = include_in_output) %>%
  filter(extended_country_name == "Yugoslavia", year %in% c(1804:1829))

vdem_simple <- vdem_simple %>% 
  anti_join(vdem_exclude)

vdem_simple <- vdem_simple %>%
  rename(vdem_country_name = country_name,
         vdem_cowcode = COWcode)

usethis::use_data(vdem_simple, overwrite = TRUE)
