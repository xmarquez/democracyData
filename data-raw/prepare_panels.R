...
countrycode_panel <- countrycode::codelist_panel |>
  janitor::clean_names() |>
  pivot_longer(
    all_of(code_types[code_types != "vdem"]),
    names_to = "code_type"
  ) |>
  relocate(country_name_en, code_type, value) |>
  ...
  summarise(
    countrycode_startdate = ymd(paste(min(year), "01-01", sep = "-")),
    countrycode_enddate = ymd(paste(max(year), "12-31", sep = "-"))
  ) |>
  group_by(code_type) |>
  mutate(
    countrycode_enddate = case_when(
      countrycode_enddate == max(countrycode_enddate) ~ NA,
      TRUE ~ countrycode_enddate
    ),
    code_type = case_when(code_type == "gwn" ~ "GWn", TRUE ~ code_type)
  ) |>
  ungroup() |>
  mutate(
    countrycode_enddate = case_when(
  ...
  arrange(
    ...
    value,
    group,
    countrycode_startdate,
    countrycode_enddate
  ) |>
  group_by(country_name_en, code_type, value, vdem, group) |>
  summarise(
    countrycode_startdate = min(countrycode_startdate),
    countrycode_enddate = case_when(
      any(is.na(countrycode_enddate)) ~ NA,
...
vdem_remaining <- vdem_panel |>
  ...
  mutate(
    country_name_en = "Palestinian Territories",
    group = vdem_group,
...
polity5_panel <- haven::read_sav(
  "https://www.systemicpeace.org/inscr/p5v2018d.sav"
) |>
...
existing_panel <- read_csv(here::here(
  "data-raw/internal_country_year_coder_data.csv"
)) |>
  pivot_longer(
    any_of(c("GWn", "extended_GWn", "cown", "polity_ccode", code_types)),
  ...
  mutate(
    matching_date = case_when(
      str_detect(code_type, "GWn") ~ "GW_",
...
not_added <- existing_panel |>
  filter(
    !extended_country_name %in% existing_panel_no_na$extended_country_name
  ) |>
  mutate(
    vdem_country_name = case_when(
      extended_country_name ==
        "British Mandate of Palestine" ~ "Palestine/British Mandate",
      TRUE ~ "Somaliland"
    )
  ) |>
...
data <- existing_panel_no_na |>
  ...
  mutate(
    startdate = case_when(
      is.na(startdate) ~ countrycode_startdate,
      TRUE ~ startdate
    ),
    enddate = case_when(is.na(enddate) ~ countrycode_enddate, TRUE ~ enddate)
  ) |>
  select(-countrycode_startdate, -countrycode_enddate, -matches("date_type")) |>
  distinct() |>
  mutate(
    extended_country_name = case_when(
      !is.na(extended_country_name) ~ extended_country_name,
    ...
    polity_membership = case_when(
      is.na(polity_membership) ~ FALSE,
      TRUE ~ polity_membership
    ),
    cow_membership = case_when(
      is.na(cow_membership) ~ FALSE,
      TRUE ~ cow_membership
    ),
    GW_membership = case_when(
      is.na(GW_membership) ~ FALSE,
      TRUE ~ GW_membership
    ),
    extended_continent = case_when(
      country_name_en == "Brunswick" ~ "Europe",
    ...
    extended_region = case_when(
      country_name_en == "Brunswick" ~ "Western Europe",
    ...
    lat = case_when(
      country_name_en == "Brunswick" ~ 52.266667,
    ...
    lon = case_when(
      country_name_en == "Brunswick" ~ 10.516667,
...
extra_fh_territories <- tibble(
  ...
  code_type = list(unique(data$code_type)),
  value = NA
) |>
  unnest(code_type)
...
data <- data |>
  ...
  mutate(across(c(regex:all_codes_NA), \(x) fill_if_unique(x))) |>
  distinct()
...
data <- data |>
  ...
  pivot_wider(
    ...
    names_from = c(code_type),
    values_from = c(value, startdate, enddate),
...
data_long <- data |>
  pivot_longer(
    any_of(c(
      union(code_types, code_types_in_data),
      "GWn",
      "extended_GWn",
      "polity_ccode"
    )),
  ...
  mutate(
    across(matches("enddate_type"), \(x) str_remove(x, "_enddate$")),
...
countrycode_panel |>
  filter(!is.na(value), code_type %in% unique(data_long$code_type)) |>
  anti_join(data_long)

data_long |>
  filter(!is.na(value), code_type %in% unique(countrycode_panel$code_type)) |>
